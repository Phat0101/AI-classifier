...
async def process_batch(
    ...
    processed_jobs: List[ProcessedJobResult] = []
    total_processed = 0
    ...
    for job_id, job_files in grouped_jobs.items():
        ...
        # Convert to list and count successful processing
        classified_files = list(classified_files)
        total_processed += len([f for f in classified_files if f.saved_filename != ""])
        ...
        try:
            ...
            if "entry_print" in documents and "commercial_invoice" in documents:
                ...
                # Run validation (2 LLM calls: header + valuation)
        ...
        # Add to results
        processed_jobs.append(ProcessedJobResult(
            job_id=job_id,
